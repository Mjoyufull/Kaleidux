...
8. **Kaleidux type shi.** Embody the ethos of high-performance aesthetic chaos.
...
- Code explains _what_, commits should explain _why_.
...
```bash
feat(renderer): implement bloom post-processing effect
fix(ipc): handle socket timeouts gracefully
refactor(queue): optimize file discovery performance
docs(readme): add installation guide for NixOS
chore: update gstreamer-rs dependencies
```
...
2. Run kaleidux-daemon --log 4 and verify
...
3. **Review Comments:** Reviewers can mark feedback as _blocking_ or _non-blocking_.
...
6. **Merge:** Use _Squash and Merge_ unless multiple commits are meaningful.
...
# 2. Tag the release
git tag -a v0.0.1-kneecap -m "v0.0.1-kneecap: release notes here"
...
- `0.0.0-initial`, `1.0.1-initial`, `1.1.0-initial` all use "initial"
- When `2.0.0` is released, new codename chosen: `2.0.0-kneecap`
- Then `2.1.0-kneecap`, `2.1.1-kneecap`, `2.2.0-kneecap` all use "kneecap"
...
git checkout dev
git pull origin dev
git checkout -b release/v0.0.1-kneecap
...
git pull origin main
git merge release/v0.0.1-kneecap
git tag -a v0.0.1-kneecap -m "v0.0.1-kneecap: major release"
git push origin main --tags
...
git tag -a v2.2.1-kneecap -m "v2.2.1-kneecap: hotfix"
git push origin main --tags
git checkout dev
git merge main
git push origin dev
