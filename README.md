...
- **High CPU or blank video wallpaper**: The production path is still appsink/WGPU. On NVIDIA, use `--video-mode cuda` to force the CUDA zero-copy decode path when auto mode cannot infer it. An experimental libmpv backend can be built with `--features mpv-backend` and selected with `--video-backend mpv`; it uses libmpv's render API software target with a reused render context to produce renderer-owned RGBA frames and is meant for correctness testing, not CPU wins yet.
