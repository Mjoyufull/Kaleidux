...
impl MpvPlayer {
    pub(super) fn spawn_event_thread(&mut self) -> anyhow::Result<()> {
        ...
        self.event_thread = Some(
            std::thread::Builder::new()
                ...
                .map_err(|error| anyhow::anyhow!("spawning libmpv event thread: {error}"))?,
        );
        Ok(())
    }

    pub(super) fn spawn_frame_thread(&mut self) -> anyhow::Result<()> {
        ...
        let Some(render_context) = self.render_context.take() else {
            ...
            anyhow::bail!("libmpv software render context is missing");
        };
        let source_id = self.source_id.clone();
        let session_id = self.session_id;
        ...
        self.frame_thread = Some(
            std::thread::Builder::new()
                ...
                .map_err(|error| anyhow::anyhow!("spawning libmpv frame thread: {error}"))?,
        );
        Ok(())
    }

    pub(super) fn spawn_native_render_thread(&mut self) -> anyhow::Result<()> {
        if self.frame_thread.is_some() {
            return Ok(());
        }
        let Some(target) = self.native_target.clone() else {
            anyhow::bail!("libmpv native render target is missing");
        };
        ...
        self.frame_thread = Some(
            std::thread::Builder::new()
                ...
                .map_err(|error| anyhow::anyhow!("spawning libmpv native thread: {error}"))?,
        );
        Ok(())
    }

    pub(super) fn spawn_composed_render_thread(&mut self) -> anyhow::Result<()> {
        if self.frame_thread.is_some() {
            return Ok(());
        }
        let Some(target) = self.composed_target.clone() else {
            anyhow::bail!("libmpv composed render target is missing");
        };
        ...
        self.frame_thread = Some(
            std::thread::Builder::new()
                ...
                .map_err(|error| anyhow::anyhow!("spawning libmpv composed thread: {error}"))?,
        );
        Ok(())
    }
}
