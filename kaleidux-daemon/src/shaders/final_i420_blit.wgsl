...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let u_raw = textureSample(t_u, samp, chroma_uv).r;
    let v_raw = textureSample(t_v, samp, chroma_uv).r;
    let y = (y_raw - uniforms.geometry_and_range.z) * uniforms.geometry_and_range.w;
    let u = (u_raw - uniforms.chroma_and_transfer.x) * uniforms.chroma_and_transfer.y;
    let v = (v_raw - uniforms.chroma_and_transfer.x) * uniforms.chroma_and_transfer.y;
